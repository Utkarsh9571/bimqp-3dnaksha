...
async function runProfile() {
  ...
  try {
    ...
    // 2. Scroll to and wait for #vr-centerpiece
    console.log('Scrolling to ImmersiveVR (#vr-centerpiece)...');
      ...
      expression: `
        new Promise((resolve) => {
          ...
          const scrollInterval = setInterval(() => {
            ...
            const vrSection = document.querySelector('#vr-centerpiece .cursor-grab');
            if (vrSection || scrollY > 12000) {
    ...
    // 3. Run continuous drag gesture on the live panorama & measure frame times
    console.log('Executing live 120-step drag-to-pan gesture on #vr-centerpiece under 4x CPU throttling...');
    const result = await client.send('Runtime.evaluate', {
      awaitPromise: true,
      returnByValue: true,
      expression: `
        new Promise((resolve) => {
          const vrViewport = document.querySelector('#vr-centerpiece .cursor-grab');
          if (!vrViewport) {
            resolve({ error: 'ImmersiveVR drag viewport not found in DOM' });
            return;
          }

          const rect = vrViewport.getBoundingClientRect();
          ...
          const steps = 120;
          const totalDuration = 2400; // 2.4 seconds continuous drag
          const stepInterval = totalDuration / steps;
          let currentStep = 0;

          // PointerDown
          vrViewport.dispatchEvent(new PointerEvent('pointerdown', {
            bubbles: true,
            clientX: startX,
            clientY: centerY,
            pointerId: 1,
          ...
          const interval = setInterval(() => {
            ...
            vrViewport.dispatchEvent(new PointerEvent('pointermove', {
              bubbles: true,
              clientX: clientX,
              clientY: centerY,
              pointerId: 1,
            ...
            if (currentStep >= steps) {
              ...
              vrViewport.dispatchEvent(new PointerEvent('pointerup', {
                bubbles: true,
                clientX: clientX,
                clientY: centerY,
                pointerId: 1,
              ...
              }, 400); // allow momentum decay to run
            }
          }, stepInterval);
        })
      `
    ...
    console.log('\n======================================================');
    console.log('   LIVE IMMERSIVE-VR DRAG PROFILING (4x CPU THROTTLED)');
    console.log('======================================================');

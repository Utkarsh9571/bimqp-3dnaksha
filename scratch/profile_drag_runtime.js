...
async function runProfile() {
  ...
  try {
    ...
    // 2. Scroll to the comparison slider section
    console.log('Scrolling to Comparison Slider...');
    await client.send('Runtime.evaluate', {
      expression: `
        (() => {
          const el = document.querySelector('.cursor-ew-resize') || document.querySelector('#experience-unbuilt');
    ...
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `
        new Promise((resolve) => {
          ...
          if (!slider) {
            resolve({ error: 'Slider element not found in DOM' });
            return;
          }

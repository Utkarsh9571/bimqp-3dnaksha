...
async function runProfile() {
  ...
  try {
    ...
    // Scroll directly to #cinematic-showcase using Lenis / DOM
    console.log('Scrolling to #cinematic-showcase...');
    ...
    // Poll until canvas is present in DOM
    console.log('Waiting for Three.js canvas mount...');
    await client.send('Runtime.evaluate', {
      awaitPromise: true,
      expression: `
        new Promise((resolve) => {
    ...
    // Measure drag orbit frame rate
    console.log('Executing live 120-step 3D model orbit gesture under 4x CPU throttling...');
    const result = await client.send('Runtime.evaluate', {
      ...
      expression: `
        new Promise((resolve) => {
          const canvas = document.querySelector('#cinematic-showcase canvas');
          if (!canvas) {
            resolve({ error: 'Canvas not found in DOM' });
            return;
          }
